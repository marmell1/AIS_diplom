...
@app.command()
def move (wh: str = typer.Option(..., help="Склад, на который перемещаем либо покупатель"),
          pr: int = typer.Option(..., help="Товар, который перемещаем"),
          q: int = typer.Option(..., help="Количество товара"),
          source: str = typer.Option( help="Склад, с которого списываем либо поставщик")):
    """
...
def read(filename: str = typer.Option(..., help="Название файла с документацией"),
    ...
    with get_session() as session1:
        ...
        if not wh1: raise ValueError("Внесите корректный номер склада")
        ...
        for key in dic_vendors.keys():
            if sheet.cell(row=dic_bill_check_vendor_row[dic_vendors[key]], column=dic_bill_check_vendor_column[dic_vendors[key]]).value == key:
                ...
                if not whl1:
                    ...
                    raise ValueError("Данная накладная уже загружена")

        if serv_b_key == 0: raise ValueError("Ошибка заполнения либо загружен документ неподдреживаемого формата")
